...
# New add
    ...
    date_expiration = fields.Date('Date expiration', track_visibility='onchange')
    date_notif = fields.Date('Date notification', track_visibility='onchange')


    @api.model
    def send_notification(self):
        events = self.search([('date_notif', '=', fields.Datetime.now())])

        # print('=============== send_notification')
        # print(fields.Datetime.now())
        # print(events_insemination)
        for event in events:
            # print('_________________ here event')
            # print(self.date_notif)
            template_id = self.env['mail.template'].search([('model_id.model', '=','muk_dms.file'),('name', '=','Notifcation documents')])[0]
            # print(template_id)
            if template_id:
                template_id.send_mail(event.id)
            # mail_message = self.env['mail.template'].send_mail(template_id[0])
